...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 13 16:10:39 2020
...
def walk(f, d, numSteps):
    """Assumes: f a field, d a Drunk in f, and numSteps and int >= 0.
...
def simWalks(numSteps, numTrials, dClass):
    """Assumes numSteps an int >= 0, numTrials and int >= 0, dClass a subclass
    of Drunk. Simulates numTrials walks of numSteps steps each. Returns a list
    of the final distanes for each trial"""
    Homer = dClass()
    origin = Location(0, 0)
    distances = []
...
def drunkTest(walkLengths, numTrials, dClass):
    ...
    dClass a subclass of Drunk. For each number of steps in walkLengths, runs
...
def plotLocs(drunkKinds, numSteps, numTrials):
    ...
    for dClass in drunkKinds:
        ...
        meanX = sum(map(abs, xVals))/len(xVals)
        meanY = sum(map(abs, yVals))/len(yVals)
        curStyle = styleChoice.nextStyle()
    ...
    plt.ylim(-1000, 1000)
    plt.xlim(-1000, 1000)
    plt.xlabel('Steps West/East of Origin')
