...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for trial in range(numTrials):
        ...
        for step in range(300):
            steps.append(patient.update())
        patients.append(steps)
    yVals = []
    for x in range(300):
        tot = 0
        for y in range(numTrials):
            tot += patients[y][x]
        yVals.append(float(tot)/numTrials)
...
#simulationWithoutDrug(500, 2000, 0.25, 0.1, 100)
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        if random.random() <= self.maxBirthProb * (1 - popDensity):
            ...
            for key, value in childResistances.items():
                if random.random() <= self.mutProb:
                    childResistances[key] = not value
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    # TODO
    virusPops, resistPops = [0]*300, [0]*300
    for trial in range(numTrials):
        virus = ResistantVirus(maxBirthProb, clearProb, resistances, mutProb)
        viruses = [virus]*numViruses
        patient = TreatedPatient(viruses, maxPop)
        for step in range(150):
            virusPops[step] += patient.update()
            resistPops[step] += patient.getResistPop(['guttagonol'])
        patient.addPrescription('guttagonol')
        for step in range(150, 300):
            virusPops[step] += patient.update()
            resistPops[step] += patient.getResistPop(['guttagonol'])
    for i in range(300):
        virusPops[i] = float(virusPops[i])/numTrials
        resistPops[i] = float(resistPops[i])/numTrials

    pylab.plot(virusPops, label='Virus Population')
    pylab.plot(resistPops, label='Virus Population\nwith Drug Resistance')
...
simulationWithDrug(100, 1000, .3, 0.2, {"guttagonol": True}, 0.1, 100)
